let app = {
    ...
    insertFiguresOnDesk() {
        ...
        for (let position of positions) {
            ...
            let cell = document.querySelector(`[data-rownum="${position.coordRow}"][data-colchar="${position.coordCol}"]`);
            // 5.2 из массива positions, для текущего объекта по индексу i, передаем название фигуры, затем цвет+"Figure"
            let figure = this.getFigure(position.figure, position.color+"Figure");
    ...
    generateBoard() {
        ...
        for (let row of this.config.rows) {
            ...
            if (rowStartWithColor === 'white') {
                ...
                rowMarkup = this.generateRow(rowStartWithColor, row);
        ...
        for (let col of this.config.cols) {
            ...
            if (currentColorClass === "white") {
                ...
                // поля в виде строки, затем номер строки игровой доски в виде числа, последним аргументом передаем this.config.cols[i]
                 field = this.generateField(currentColorClass, rowNum, col);
                ...
                // 2.7 делаем то же что и в п. 2.5, только первым параметром передаем "black"
                 field = this.generateField('black', rowNum, col);
