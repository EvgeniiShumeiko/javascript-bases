...
function handleClick(clickedButtonEvent) {
    ...
    const cardButton = clickedButtonEvent.target;
    const cardNode = cardButton.parentNode;
    const cardImage = cardNode.querySelector('img');
    const cardName = cardNode.querySelector('.productName');

    // 3. создайте литерал объекта со следующими свойствами:
    const card = {
        wrap: cardNode, // здесь элемент с классом .product
        img: cardImage, // здесь картинка внутри .product
        productName: cardName, // здесь .productName, который внутри .product
    ...
    if (cardButton.innerText === 'Подробнее') { // 4.1 проверяем равняется ли этот текст строке "Подробнее"
        // 4.2 если да, то передаем объект card в функцию showMoreText
        showMoreText(card);
    } else if (cardButton.innerText === 'Отмена') { // 4.3 проверяем равняется ли текст на кнопке строке "Отмена"
...
function showMoreText(card) {
    ...
    let text = "Lorem ipsum dolor sit amet, consectetur adipisicing elit. Ab at inventore ipsam natus nesciunt nihil officiis placeat provident sit voluptatum?"
